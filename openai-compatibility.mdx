...
Your full API key looks like `dpn-sk-<id>|<secret>` — the `dpn-sk-` prefix is required by the gateway, and applies only here (the native API at `https://api.deapi.ai` takes the raw key). You can find it in your [Dashboard → Settings → API Keys](https://app.deapi.ai/dashboard).
