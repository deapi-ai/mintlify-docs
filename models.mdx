...
<Warning>
  **Send `?limit=50` or you will not see every model.** Without it the endpoint returns only the first **25** models and nothing in the response looks like an error — the newest models are simply missing. `limit=50` is the maximum page size and covers the current catalogue in one request.

  The parameter is **`limit`**. `per_page` is accepted but ignored and leaves the page at 25. A value above 50 is clamped to 50, not rejected.

  `meta.last_page` tells you whether one call was enough: it reads `1` for a complete list. Should the catalogue ever outgrow 50 entries, that value becomes your signal to fetch `&page=2`.
</Warning>
