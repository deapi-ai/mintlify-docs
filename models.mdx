...
| Text-to-Image /<br />Image-to-Image | Generate images from text / edit existing image | Fast text-to-image generation and multi-reference image editing in a single unified model. Ideal for real-time applications, local development, and edge deployment. | FLUX.2 Klein 4B BF16     | `Flux_2_Klein_4B_BF16`         | `txt2img`<br />`img2img` |
...
| `Flux_2_Klein_4B_BF16`         | txt2img, img2img     | 16              | 256, 512, 768, 1024, 1536 |
...
- **Image Generation (Text-to-Image):** Start with `Flux_2_Klein_4B_BF16`, `Flux1schnell` or `ZImageTurbo_INT8` for fast iteration. Increase steps/resolution for quality; control style via prompt and (optionally) LoRAs.
...
- **Image Transformation (Image-to-Image):** For edits and style transfer, start with `Flux_2_Klein_4B_BF16`. Use fewer steps for quick drafts and increase steps for higher fidelity; combine with masks or control prompts for targeted changes.
