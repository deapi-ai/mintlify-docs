...
The [Model Selection](/api/v2/utilities/models) endpoint returns the live, authoritative list of available models, their slugs, limits, and defaults. **Use it as the starting point for every integration.** The list is paginated — see [Fetching the live model list](#fetching-the-live-model-list) for how to read every page.
...
```bash
curl -X GET "https://api.deapi.ai/api/v2/models" \
...
<Warning>
  **The response is paginated — one call is not the whole catalogue.** A single request returns at most **25** models, alongside `links` and `meta` objects. If `meta.last_page` is greater than `1`, request the remaining pages and concatenate the results; otherwise newer models are silently missing from your list. The `per_page` parameter is accepted but currently ignored — the page size stays at 25 whatever you send.
</Warning>

Keep requesting successive pages until `meta.current_page` equals `meta.last_page`:

```bash
curl -X GET "https://api.deapi.ai/api/v2/models?page=2" \
  -H "Authorization: Bearer $DEAPI_API_KEY" \
  -H "Accept: application/json"
```
...
curl -X GET "https://api.deapi.ai/api/v2/models?filter[inference_types]=txt2img" \
  -H "Authorization: Bearer $DEAPI_API_KEY" \
  -H "Accept: application/json"
```
...
Always read the information about limits and default values ​​for individual models from the model endpoint and check the current price in the endpoint price-calculation for the given model.
...
1. **Call** `GET /api/v2/models` at the start of your session to get the current model list. Do not rely on model slugs from training data, cached documentation, or prior conversations — they may be outdated.
