...
| ------------------------------------- | ----------------------------------- | ------------------------------------------------------ | ----------------------------------------------------------------------------------- |
| Text-to-Image (Flux.1 schnell)        | resolution × steps                  | $0.00136 for 512x512, 4 steps                          | Uses Flux Schnell as baseline in sample calculator                                  |
| Text-to-Image (Z-Image-Turbo INT8)    | resolution × steps                  | $0,00405 for 512x512, 4 steps                          | Generates very realistic images                                                     |
| Image-to-Image                        | steps (style transfer)              | ~$0.0132 for 512x512, 20 steps                         | Cost scales with steps and GPU time                                                 |
| Text-to-Speech (TTS)                  | number of characters                | $0.77 per 1 M characters                               | Adjustable speed multipliers (fast = 0.5× cost, slow = 2× cost)                     |
| Text-to-Video                         | video duration + resolution         | $0.001737 for 2s, 256×256                              | 2–5 second clips; higher res or steps increase cost                                 |
| Image-to-Video                        | source image + motion interpolation | $0.001737 for 2s, 256×256                              | 2–5 second output, smooth motion                                                    |
| Video-to-Text (YouTube Transcription) | video length                        | $0.021 per hour                                        | Supports timestamps, multilingual                                                   |
| Image-to-Text (OCR)                   | output characters                   | $0.00928 per 1,000 output chars (for 1024×1024 images) | Also includes object detection, scene understanding                                 |
| Text-to-Embedding                     | number of tokens processed          | $0.000068 per 1,000 tokens                             | Supports large-scale semantic search and RAG; cost scales linearly with token count |
...
- Public example: Flux Schnell model is used to estimate cost in the UI; for example, 512x512 at 4 steps gives **$0.00136**. For the Z-Image-Turbo INT8 model with the same parameters, the price is **$0.00405**, but the advantage is very realistic images.
...
- Charged per character in your input (e.g. 1M characters → **$0.77**).
...
- Example public rate: **$0.001737** for a 2-second clip at 256x256.
...
- Example: 256x256 for 2 seconds costs **$0.001737**.
...
- Sample public rate: **$0.021 per hour**.
- For 5 minutes, cost is estimated at **$0.003613**.
...
- Baseline rate: **$0.00928** per 1,000 output characters (for 1024×1024 images).
- Example outputs:
  - Single photo (≈20 chars) → **$0.000186**
  - Math expression (~350 chars) → **$0.0032**
  - Book page (~1,500 chars) → **$0.0139**
...
**Link to live pricing page for reference:**
