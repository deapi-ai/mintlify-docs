...
- Use the `/api/v2/videos/upscales/price` endpoint with your `model`, `width`, `height`, `duration`, and optional `scale` to get exact pricing before uploading the file.
- **`duration` is not optional in practice.** The endpoint still returns `200` when you leave it out, but the quote is then `price: 0` — the price is proportional to duration, so a missing duration means no price at all. The field is spelled `duration`; `duration_seconds` (the transcription field name) is ignored here and produces the same `0`. See [Video Upscale Price Calculation](/api/v2/videos/upscales-price).
