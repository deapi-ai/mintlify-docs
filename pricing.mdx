...
| Word timestamps \+ diarization add-on (WhisperLargeV3Ct2) | \+50% of transcription price | segment timestamps are free | `ts_level: "word"`, `diarize: true` |
...
Some models in the catalogue run on our partners' infrastructure rather than on deAPI's own GPU network — see [Partner (external) models](/models#partner-external-models). You can recognise them by the `experimental` tag and `status: test_model` in the model list. They are billed differently from native fleet models.
...
Native fleet models are priced from a formula over your request parameters, so the quote is exact. Partner models are priced by the partner, so deAPI quotes an estimate up front and reconciles it once the partner reports the actual cost.
...
A failed job is normally refunded — `refunded: true` and `price.amount: 0`. Note that `refunded: false` covers two different situations: a charge deliberately kept, and a job that failed before it was ever charged. It does not by itself mean you were billed.
...
- For reference inputs, send `ref_images_count` (0–9, default `0`) and `ref_videos_count` (0–1, default `0`) to [Video Animation Price Calculation](/api/v2/videos/animations-price); no file upload is needed for a quote. The source image does not count as a reference image.
- More than 2 reference images applies a **×2** multiplier; any reference video applies **×3**. Only the highest multiplier applies — 4 reference images plus 1 video is **×3**, not ×6. Exactly 2 reference images adds no surcharge, and reference audio adds no surcharge.
...
`WhisperLargeV3Ct2` adds word-level timestamps, per-segment confidence scores and speaker diarization. It is also the only model that transcribes TikTok — a TikTok URL sent with any other model is rejected with `422`. <br />The price list for TikTok transcription depends on the video length and is as follows:
...
Every other source — YouTube, X, Twitch, Kick, uploaded files — is billed by duration and runs on either model.
