...
| `POST` | `/api/v1/client/vid2txt` | Transcribe video from URL (YouTube, X) | [→](/api/analysis/video-to-text) |
...
| `POST` | `/api/v1/client/vid-rmbg` | Remove video background | *Coming soon* |
| `POST` | `/api/v1/client/vid-rmbg/price-calculation` | Calculate video background removal price | *Coming soon* |
| `POST` | `/api/v1/client/vid-upscale` | Upscale video resolution | *Coming soon* |
| `POST` | `/api/v1/client/vid-upscale/price-calculation` | Calculate video upscale price | *Coming soon* |
...
| `GET` | `/api/v1/client/request-status/{job_request}` | Check job status and get results | [→](/api/utilities/get-results) |
...
<Steps>
  ...
  <Step title="Submit job">
    Call the main endpoint (e.g., `POST /api/v1/client/txt2img`) with your parameters.
    Response contains `request_id`.
  </Step>
  <Step title="Poll for results">
    Call `GET /api/v1/client/request-status/{job_request}` until `status` is `done` or `failed`.
    Use the `request_id` from step 3 as the `job_request` path parameter.
  </Step>
