...
<Note>
  `platform: "tiktok"` is only valid with `model: "WhisperLargeV3Ct2"` — that is the only model that transcribes TikTok. Paired with any other model it is rejected with `422`, the same way the job itself would be. See [Audio/Video Transcription](/api/v2/audio/transcriptions).
</Note>
