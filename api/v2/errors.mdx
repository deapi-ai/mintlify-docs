---
title: "Errors"
description: "HTTP status codes and error responses returned by deAPI v2"
---
...
| `403` | Forbidden             | Account suspended. Returned before validation on endpoints that spend credits; `/price` endpoints are not gated. |
...
**Unknown job / resource** — e.g. an invalid `request_id` when polling results:
...
```json
{
  "message": "The route api/v2/utilities/balance could not be found."
}
```

**Solution:** Verify the `request_id`, and take API paths from the [API Overview](/api/v2/overview) — documentation URLs such as `/api/v2/utilities/balance` are not API routes (the real one is `/api/v2/account/balance`).
...
**Solution:** Iterate the keys of the `errors` object to identify which fields failed and correct them. Many limits are model-specific — read `info.limits` from the [Model Selection](/api/v2/utilities/models) endpoint for the model you are calling.
