...
<Info>
  In v2 all prompt boosters use a **single unified endpoint** — `POST /api/v2/prompts/enhancements`. You select the booster with the **`type`** field (v2 dot notation) and the target model via **`model_slug`**. (Sample prompts remain on the stable `/api/v1/client/prompts/samples` path.)
</Info>
...
| Sample Prompts | `GET  /api/v1/client/prompts/samples` |
