...
| `GET` | `/api/v2/models` | List available models (paginated, 25 per page) | [→](/api/v2/utilities/models) |
...
<Steps>
  <Step title="Check available models">
    Call `GET /api/v2/models` to get current models and their parameters. The response is paginated at 25 models per page — keep requesting `?page=N` until `meta.current_page` equals `meta.last_page`.
  </Step>
...
```json Models List (paginated — max 25 per page)
{
  "data": [
    {
  ...
  "links": {
    ...
    "last": "https://api.deapi.ai/api/v2/models?page=2",
    "prev": null,
