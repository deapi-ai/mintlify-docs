...
| `GET` | `/api/v2/models` | List available models — send `?limit=50` for the full list | [→](/api/v2/utilities/models) |

<Warning>
  **Call the path in the "Endpoint" column, not the documentation URL.** The links in the "Docs" column are addresses on this documentation site and are grouped by topic — `/api/v2/utilities/balance`, `/api/v2/utilities/models`, `/api/v2/utilities/jobs` and `/api/v2/audio/music-price` are page URLs, not API routes, and calling them against `api.deapi.ai` returns `404 The route ... could not be found.` The real routes are `/api/v2/account/balance`, `/api/v2/models`, `/api/v2/jobs/{job_request}` and `/api/v2/audio/music/price`.
</Warning>
...
<Steps>
  <Step title="Check available models">
    Call `GET /api/v2/models?limit=50` to get current models and their parameters. `limit=50` is the maximum page size and returns the whole catalogue in one call — without it you get only the first 25 models and no error to tell you the rest are missing.
  </Step>
...
```json Models List (GET /api/v2/models?limit=50)
{
  "data": [
    {
  ...
  "links": {
    ...
    "last": "https://api.deapi.ai/api/v2/models?page=1",
    "prev": null,
    "next": null
  },
  "meta": {
