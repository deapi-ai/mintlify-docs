...
<Warning>
**Send `limit=50` and check `meta.last_page`.** The page size defaults to 50 and is capped at 75 — a larger value is clamped down, not rejected. `limit=50` covers the current catalogue in one call, and `meta.last_page` reading `1` is what confirms the `data` array is complete; nothing else in the response distinguishes a partial page from a full one. `per_page` does not work for this; it is accepted and ignored.
</Warning>
