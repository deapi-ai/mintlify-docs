...
Each window opens on your first request into a bucket and runs its full length from that moment — not to a calendar boundary in any timezone. Read `X-RateLimit-Reset` (RPM) and `X-RateLimit-Daily-Reset` (RPD) for the exact epoch second.
...
| --- | --- | --- | --- |
| `/api/v2/images/generations` | `Flux1schnell`, `ZImageTurbo_INT8`, `Flux_2_Klein_4B_BF16`, `ZAnimeDistill_8Step_INT8` | 3 | 100 |
...
| `/api/v2/videos/generations` | `Ltxv_13B_0_9_8_Distilled_FP8`, `Ltx2_3_22B_Dist_INT8`, `Ltx2_5_22B_Dist_INT8` | 1 | 15 |
| `/api/v2/videos/generations` | `MiniMaxH3\_33B\_Turbo\_INT8` | 1 | 3 |
| `/api/v2/videos/animations` | `Ltxv_13B_0_9_8_Distilled_FP8`, `Ltx2_3_22B_Dist_INT8`, `Ltx2_5_22B_Dist_INT8` | 1 | 15 |
| `/api/v2/videos/animations` | `MiniMaxH3\_33B\_Turbo\_INT8` | 1 | 3 |
| `/api/v2/videos/audio-syncs` | `Ltx2_3_22B_Dist_INT8`, `Ltx2_5_22B_Dist_INT8` | 1 | 15 |
| `/api/v2/videos/replacements` | `Wan2_2_Animate_14B_INT8` | 1 | 15 |
| `/api/v2/videos/upscales` | `RealESRGAN_Vid_x4 , RealESRGAN_Vid_x2 , FlashVSR_Tiny` | 1 | 15 |
| `/api/v2/audio/speech` | `Kokoro`, `Chatterbox`, `Qwen3_TTS_12Hz_1_7B_CustomVoice`, `Qwen3_TTS_12Hz_1_7B_Base`, `Qwen3_TTS_12Hz_1_7B_VoiceDesign` | 5 | 300 |
| `/api/v2/audio/music` | `AceStep_1_5_Turbo`, `AceStep_1_5_Base`, `AceStep_1_5_XL_Turbo_INT8` | 1 | 15 |
| `/api/v2/audio/transcriptions` | `WhisperLargeV3`, `WhisperLargeV3Ct2` | 1 | 10 |
...
| Endpoint | Models | RPM | RPD |
| --- | --- | --- | --- |
| `/api/v2/images/generations` | `Flux1schnell`, `ZImageTurbo_INT8`, `Flux_2_Klein_4B_BF16`, `ZAnimeDistill_8Step_INT8` | 300 | Unlimited |
...
| `/api/v2/videos/generations` | `Ltxv_13B_0_9_8_Distilled_FP8`, `Ltx2_3_22B_Dist_INT8`, `Ltx2_5_22B_Dist_INT8`, `MiniMaxH3_33B_Turbo_INT8` | 300 | Unlimited |
| `/api/v2/videos/animations` | `Ltxv_13B_0_9_8_Distilled_FP8`, `Ltx2_3_22B_Dist_INT8`, `Ltx2_5_22B_Dist_INT8`, `MiniMaxH3_33B_Turbo_INT8` | 300 | Unlimited |
| `/api/v2/videos/audio-syncs` | `Ltx2_3_22B_Dist_INT8`, `Ltx2_5_22B_Dist_INT8` | 300 | Unlimited |
| `/api/v2/videos/replacements` | `Wan2_2_Animate_14B_INT8` | 300 | Unlimited |
| `/api/v2/videos/upscales` | `RealESRGAN_Vid_x4 , RealESRGAN_Vid_x2 , FlashVSR_Tiny` | 300 | Unlimited |
| `/api/v2/audio/speech` | `Kokoro`, `Chatterbox`, `Qwen3_TTS_12Hz_1_7B_CustomVoice`, `Qwen3_TTS_12Hz_1_7B_Base`, `Qwen3_TTS_12Hz_1_7B_VoiceDesign` | 300 | Unlimited |
| `/api/v2/audio/music` | `AceStep_1_5_Turbo`, `AceStep_1_5_Base`, `AceStep_1_5_XL_Turbo_INT8` | 300 | Unlimited |
| `/api/v2/audio/transcriptions` | `WhisperLargeV3`, `WhisperLargeV3Ct2` | 300 | Unlimited |
...
Utility endpoints are limited generously rather than not at all, and they sit in their own buckets:

- `/api/v2/account/balance` — 600 RPM (`X-RateLimit-Source: static`, identical for every account)
- `/api/v2/models` — 300 RPM on Premium (`X-RateLimit-Source: tier`)

Read `X-RateLimit-Limit` on any response rather than hard-coding these.
...
- Max size: **20 MB**
- Supported Formats: AAC, MP3, OGG WebM
- Voice Cloning reference audio: max **15 MB**, 3–10 seconds duration. Supported formats: MP3, OGG, M4A.
- Audio-to-Video conditioning audio: max **20 MB**. Supported formats: MP3, OGG.
- Music reference audio (style transfer): max **15 MB**. Supported formats: MP3, OGG, M4A.
- Image-to-Video reference audio (`ref_audios`): max **15 MB** per clip. Supported formats: WAV, MP3, FLAC, OGG, M4A. Duration is model-specific (default range 1–10 seconds); audio must accompany at least one reference image or video.
...
- Image-to-Video keyframe and reference images must be at least **1 KB** per file.
- `ref_images` uses the same image size, format, and dimension limits above, with an aspect ratio within **1:4–4:1**. First/last keyframes may have any aspect ratio because they are fitted to the generation canvas.
- Reference counts and durations depend on the selected model. Read `info.limits.max_ref_images`, `max_ref_videos`, `max_ref_audios`, and `max_ref_files`, plus `min_ref_video_duration`, `max_ref_video_duration`, `min_ref_audio_duration`, `max_ref_audio_duration`, `min_ref_image_ratio`, and `max_ref_image_ratio`. An unsupported reference type has a count limit of `0`.
- Outer ceilings are **9 images**, **1 video**, **3 audio clips**, and **12 reference files in total**. Reference video duration defaults to **2–5 seconds**, but the model's published limits take precedence. The global **75 MB** request cap still applies.
- References cannot be combined with `first_frame_image` or `last_frame_image`. See [Video Animation](/api/v2/videos/animations) for input modes and ordering rules.

<Tip>
  For larger source media that won't fit under the request body cap, prefer **URL-based input** (`source_url`) where supported — it bypasses the upload cap entirely and is subject only to the duration limits listed below.
</Tip>
...
```bash
GET https://api.deapi.ai/api/v2/models?limit=50
```
...
2. **RPD exceeded** — Wait for the delay in `Retry-After`, or the epoch second in `X-RateLimit-Daily-Reset`
