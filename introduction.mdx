...
- **Video AI** — Generation, animation, audio-sync, character replacement, upscaling, and transcription from YouTube, Twitch, Kick, TikTok and X
...
- **Enhancement** — Upscale images and videos to higher resolution; remove image backgrounds
...
deAPI exposes an OpenAI-compatible endpoint at `https://oai.deapi.ai/v1`. Change two parameters (`base_url` and `api_key`), keep your existing OpenAI SDK code, and route image generation, TTS, transcription, embeddings, and video generation to deAPI.

<Card icon="plug" href="/openai-compatibility" title="OpenAI Compatibility">
  Drop-in replacement for the OpenAI SDK — migrate in two lines
</Card>
