...
Configure a global webhook URL in your [account settings](https://app.deapi.ai/settings/webhooks). All jobs will send notifications to this URL unless overridden.
...
```json
{
  ...
  "webhook_secret": "your-per-job-secret-32-to-255-chars"
}
```
...
<Tabs>
  <Tab title="job.processing">
    ...
    ```json
    {
      ...
      "delivery_id": "9633a858-475f-45b0-98df-5fd7027cef2d",
      "timestamp": "2026-08-07T11:43:14+00:00",
      "data": {
        "job_request_id": "868543c4-9429-4434-972a-db27769c8a34",
        "status": "processing",
        "previous_status": "pending",
        "job_type": "txt2img",
        "started_at": "2026-08-07T11:43:14+00:00"
      }
    }
    ```
    ...
    | `data.job_type` | string | The job's **inference type slug** — the same value the models endpoint reports in `inference_types` (e.g. `txt2img`, `img2img`, `txt2video`, `vid-upscale`, `video2text`). Not the endpoint name. |
  ...
  <Tab title="job.completed">
    ...
    {
      "event": "job.completed",
      "delivery_id": "c4d543d8-b52b-4f62-a6c1-5605111dd830",
      "timestamp": "2026-08-07T11:43:26+00:00",
      "data": {
        "job_request_id": "868543c4-9429-4434-972a-db27769c8a34",
        "status": "done",
        "previous_status": "processing",
        "job_type": "txt2img",
        "completed_at": "2026-08-07T11:43:26+00:00",
        "result_url": "https://results.deapi.ai/19315424/....png?X-Amz-Signature=...",
    ...
    | `data.job_type` | string | The job's inference type slug (see the `job.processing` tab) |
    | `data.completed_at` | string (ISO 8601) | When job completed |
    | `data.result_url` | string (URL) | **Pre-signed, expiring** URL to download the result. Fetch it promptly, or re-read a fresh URL from `GET /api/v2/jobs/{request_id}`. |
    | `data.processing_time_ms` | integer | Processing time in milliseconds |

    <Note>
      The webhook payload carries only `result_url`. Alternative renditions (`results_alt_formats`, e.g. `jpg` / `webp` for images) and inline results are available from `GET /api/v2/jobs/{request_id}`.
    </Note>
  </Tab>
  <Tab title="job.failed">
    ...
    ```json
    {
      ...
      "timestamp": "2026-08-07T11:43:45+00:00",
      "data": {
        "job_request_id": "868543c4-9429-4434-972a-db27769c8a34",
        "status": "error",
        "previous_status": "processing",
        "job_type": "txt2img",
        "failed_at": "2026-08-07T11:43:45+00:00",
    ...
    | `data.error_code` | string | Coarse failure category |
    | `data.error_reason` | string | Actionable failure reason — **open list**, see below |
    | `data.retryable` | boolean | Whether resubmitting the identical request could succeed |
    | `data.refunded` | boolean | Whether the upfront charge was returned |

    **`error_code` — coarse category:**

    | Code | Description |
    | --- | --- |
    | `WORKER_TIMEOUT` | Worker didn't respond in time |
    | `WORKER_ERROR` | The worker itself failed |
    | `PROCESSING_ERROR` | Error during inference |

    **`error_reason` — actionable reason:**

    | Reason | Description |
    | --- | --- |
    | `SOURCE_PRIVATE` | The source is private, deleted or otherwise unavailable to us |
    | `SOURCE_RESTRICTED` | An age or region gate blocks it |
    | `SOURCE_UNPROCESSABLE` | Fetched, but the media is unusable (e.g. no audio track) |
    | `SOURCE_DOWNLOAD_FAILED` | The source could not be downloaded |
    | `INPUT_TOO_LARGE` | Input exceeded the model's limits |
    | `TIMEOUT` | The job exceeded its time budget |
    | `CAPACITY_UNAVAILABLE` | No capacity was available for the model |
    | `PROCESSING_FAILED` | Inference failed for another reason |

    <Warning>
      **`error_reason` is an open list.** New values can be added without a major version bump, so treat any unrecognised value as a generic failure rather than crashing. Branch your retry logic on `data.retryable`, not on `error_reason` — `retryable` stays correct as new reasons are added.
    </Warning>

    <Note>
      `refunded: false` covers two different situations: a charge that was deliberately kept, and a job that failed before it was ever charged. It does not by itself mean you were billed.
    </Note>
  </Tab>
</Tabs>
...
If neither a per-request `webhook_secret` nor an account-default secret exists, the callback is still delivered but **unsigned** — `X-DeAPI-Signature` arrives empty. Reject an empty value if you require authenticity.

<Info>
  If you supplied a `webhook_secret` in the original job request, every callback for that job (including all retries) is signed with **that** secret. Otherwise, the signature uses the global webhook secret from your [account settings](https://app.deapi.ai/settings/webhooks). Your verification code must use whichever secret you sent for that job — typically by looking it up from `data.job_request_id` in your own store.
</Info>
