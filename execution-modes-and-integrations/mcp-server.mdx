...
| `video_url_transcription`        | Transcribe videos from URLs (YouTube, X, Twitch, TikTok, Kick) — uses `/api/v2/audio/transcriptions`. For TikTok set `model` to `WhisperLargeV3Ct2`; the tool otherwise defaults to `WhisperLargeV3`, which rejects TikTok |
