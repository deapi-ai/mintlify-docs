...
**Long transcripts.** Some MCP clients cap how much of a tool result they pass to the model and drop the rest. For long videos, call the transcription tool with `return_result_in_response: false`: the transcript stays out of the tool result and the tool returns `result_url`, a signed link to the full transcript file.
