...
- A **deAPI API key** from the [deAPI Dashboard](https://app.deapi.ai/dashboard)
...
+----------+----------+
           ...
           v
+---------------------+
|    HTTP Request     |  POST /api/v2/images/generations
|    (Submit Job)     |
+----------+----------+
           |
           v
...
|---------|-------|
| **Method** | `GET` |
| **URL** | `https://api.deapi.ai/api/v2/jobs/{{ $node["HTTP Request"].json.data.request_id }}` |
| **Authentication** | Generic Credential Type |
| **Generic Auth Type** | Bearer Auth |
| **Bearer Auth** | Select your deAPI credential |
...
| Transcription             | `/api/v2/audio/transcriptions`             |
| Embeddings                | `/api/v2/embeddings`                       |

All endpoints follow the same pattern:

1. `POST` to submit the job → receive `request_id`
2. `GET /api/v2/jobs/{request_id}` → poll status
...
- [Model Selection](/api/v2/utilities/models)
