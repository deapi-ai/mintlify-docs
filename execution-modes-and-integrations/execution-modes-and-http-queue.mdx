...
description: "How to receive job results via webhooks, WebSockets, or polling"
---
...
With webhooks, deAPI sends an HTTP POST to your server when a job completes. Configure a global webhook URL in [account settings](https://app.deapi.ai/settings/webhooks), or override per-request:
...
```json
{
  ...
  "data": {
    ...
    "status": "done",
    "job_type": "txt2img",
    "result_url": "https://results.deapi.ai/.../output.png",
...
# 2. Poll for results
curl https://api.deapi.ai/api/v2/jobs/abc123 \
  -H "Authorization: Bearer $API_KEY"
```
...
| `error_code`, `error_message`, `error_reason`, `retryable`, `refunded` | Error details (when `error`) |
...
→ [Get Job Results Endpoint Reference](/api/v2/utilities/jobs)
...
| ------------------------------------------------ | ---------------------------- |
| `POST /api/v2/{resource}/{operation}`            | Submit a job                 |
| `GET /api/v2/jobs/{request_id}`                  | Check job status (polling)   |
