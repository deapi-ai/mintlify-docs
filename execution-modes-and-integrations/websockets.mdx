---
title: 'WebSockets'
description: 'Receive real-time job status updates via WebSocket connections'
---
...
- **Client ID:** Your unique identifier (from [dashboard](https://app.deapi.ai/dashboard))
- **API Token:** Your authentication token (see [Quickstart](/quickstart))
...
| `error` | Failed | Via [webhooks](/execution-modes-and-integrations/webhooks) only |
...
<AccordionGroup>
  ...
  <Accordion title="Connection drops frequently">
    ...
    - Consider using [webhooks](/execution-modes-and-integrations/webhooks) as a backup
  </Accordion>
</AccordionGroup>